...
def train(generator, discriminator, dataloader, opt):
    ...
    start = time.time()
    maxIS = 0
    fixed_z = Variable(Tensor(np.random.normal(0, 1, (1, opt.latent_dim))))
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, (imgs, _) in enumerate(dataloader, batches_done % len(dataloader)):
            ...
            fake_imgs, x_in, x_up, x_out = generator(z)
            ...
            if i % opt.n_critic == 0:
                ...
                if batches_done % opt.sample_interval == 0:
                    fake_imgs, x_in, x_up, x_out = generator(fixed_z)
                    if batches_done == 0:
                        vutils.save_image(real_imgs.data[:49], (os.path.join(saveDir, opt.dataset + "_real.png")), nrow=7, normalize=True)
                        vutils.save_image(fake_imgs.data, (os.path.join(saveDir, opt.dataset + "_fake_%s.png")) % str(batches_done).zfill(8), normalize=True)
                    else:
                        vutils.save_image(fake_imgs.data, (os.path.join(saveDir, opt.dataset + "_fake_%s.png")) % str(batches_done).zfill(8), normalize=True)
                        vutils.save_image(x_in[1].data, (os.path.join(saveDir, opt.dataset + "_xin_%s.png")) % str(batches_done).zfill(8), normalize=True)
                        vutils.save_image(x_up[1].data, (os.path.join(saveDir, opt.dataset + "_xup_%s.png")) % str(batches_done).zfill(8), normalize=True)
                        vutils.save_image(x_out[1].data, (os.path.join(saveDir, opt.dataset + "xout_%s.png")) % str(batches_done).zfill(8), normalize=True)
