...
def train(generator, discriminator, dataloader, opt):
    ...
    if opt.resume != 0:
        ...
        generator.load_state_dict(torch.load(os.path.join(opt.loadDir, "generator_model_%s") % str(opt.resume).zfill(8)))
        discriminator.load_state_dict(torch.load(os.path.join(opt.loadDir, "discriminator_model_%s") % str(opt.resume).zfill(8)))
